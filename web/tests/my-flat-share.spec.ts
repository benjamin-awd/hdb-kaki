...
test('a deep link restores the form and computes a valuation', async ({ page }) => {
  ...
  // ...and the URL-supplied area survives resolveBlock()/onFlatChange()'s auto-fill,
  // which is the ordering bug this restore path guards against.
  await expect(page.locator('#f-area')).toHaveValue('990');
  // Remaining lease is derived from the dataset (not user-editable), so it just needs
  // to resolve off its "—" placeholder into a computed value.
  await expect(page.locator('#f-lease')).not.toHaveText('—');
