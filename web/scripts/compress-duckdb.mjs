...
for (const f of wasm) {
  ...
  console.log(
    `brotli ${f}: ${(raw.length / 1048576).toFixed(1)} → ${(br.length / 1048576).toFixed(1)} MiB`,
  );
}
...
for (const platform of ['wasm_eh', 'wasm_mvp']) {
  ...
  try {
    ...
    console.log(
      `brotli ${platform}/${name}: ${(raw.length / 1024).toFixed(0)} → ${(br.length / 1024).toFixed(0)} KiB`,
    );
  } catch (err) {
    console.warn(
      `WARNING: could not stage ${platform} parquet extension (${err.message}); ` +
